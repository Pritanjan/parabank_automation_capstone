...
@then(parsers.parse('I should see a approval message "{message}"'))
def verify_confirmation_message(driver, message):
    loan_page = LoanApplicationPage(driver)
    result = loan_page.get_result_message()
    
    assert result is not None, "Expected a result message but got None"


@then(parsers.parse('I should see an denial message "{message}"'))
